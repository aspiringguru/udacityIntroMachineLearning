...
for featImp in clf.feature_importances_:
    if featImp >= 0.2:
        print count, featImp, vectorizer.get_feature_names()[count]
        temp = vectorizer.get_feature_names()[count]
        results.append((count, featImp, temp))
        if featImp > maxFeatImp:
            maxFeatImp = featImp
            maxCount = count
        #NB mabe another container type as list of tuples not best?
    count += 1
print "maxFeatImp=", maxFeatImp, "count=",count
...
print "clf.feature_importances_.argmax()=", clf.feature_importances_.argmax()
...
for counter, importance in enumerate(clf.feature_importances_):
    if importance > 0.2:
        print "Counter =", counter, " importance=", importance, " vectorizer.get_feature_names()["+str(counter)+"]=", vectorizer.get_feature_names()[counter]
#correct answers are 0.764705882353 & 33614